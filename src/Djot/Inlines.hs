...
consolidate (Inlines ils') = Inlines (foldr go mempty ils')
 where
   go il@(Node (Attr []) (Str s)) ils
      | not (B8.null s)
      , not (isWs (B8.last s))
      = case Seq.viewl ils of
              Node attr (Str s') Seq.:< end
                -> Node attr (Str (s <> s')) Seq.<| end
              _ -> il Seq.<| ils
   go il ils = il Seq.<| ils
